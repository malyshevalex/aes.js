...
const encryptionKey = userKey => {
  const rounds = getRounds(userKey)
  const key = new Uint32Array(4 * (rounds + 1))
  let rk = key.subarray(0)
  let i = 0

  rk[0] = userKey.getUint32(0)
  rk[1] = userKey.getUint32(4)
  rk[2] = userKey.getUint32(8)
  rk[3] = userKey.getUint32(12)
  ...
  rk[4] = userKey.getUint32(16)
  rk[5] = userKey.getUint32(20)
  if (rounds === 12) {
    while (true) {
      const temp = rk[5]
      rk[6] = rk[0] ^
  ...
  rk[6] = userKey.getUint32(24)
  rk[7] = userKey.getUint32(28)
  if (rounds === 14) {
    while (true) {
      let temp = rk[7]
      rk[8] = rk[0] ^
...
const decryptionKey = (userKey, encKey) => {
  const rounds = getRounds(userKey)
  const key = encryptionKey ? encKey.slice() : encryptionKey(userKey)

  /* invert the order of the round keys: */
  for (let i = 0, j = 4 * rounds; i < j; i += 4, j -= 4) {
    swap(key, i, j)
    swap(key, i + 1, j + 1)
    swap(key, i + 2, j + 2)
    swap(key, i + 3, j + 3)
...
export default class AES {
  ...
  /**
   * Creates new AES instance with provided key
   * @param {TypedArray|ArrayBuffer} key 
   */
  constructor(key) {
    if (key.byteLength !== 16 && key.byteLength !== 24 && key.byteLength !== 32) {
      throw 'Key size should be 16, 24 or 32 bytes'
    }
    Object.defineProperties(this, {
      key: { value: arrconv(key, DataView, true) },
      BlockSize: { value: 16 }
    })
  }
   ...
   * @returns {DataView} encrypted buffer view
   */
  encrypt(data, inPlace = false) {
    if (data.byteLength !== this.BlockSize) {
      throw 'Expeceted 16 bytes block'
    }
    if (!this._eKey) {
      this._eKey = encryptionKey(this.key)
    }
    const ret = arrconv(data, DataView, !inPlace)

    let rk = this._eKey.subarray(0)
    const rounds = rk.length / 4 - 1

    let s0 = ret.getUint32(0) ^ rk[0]
    let s1 = ret.getUint32(4) ^ rk[1]
    let s2 = ret.getUint32(8) ^ rk[2]
    let s3 = ret.getUint32(12) ^ rk[3]
    const r = rounds / 2
    let  i = 0
    let t0, t1, t2, t3
   ...
   */
  decrypt(data, inPlace = false) {
    if (!this._dKey) {
      this._dKey = decryptionKey(this.key, this._eKey)
    }
